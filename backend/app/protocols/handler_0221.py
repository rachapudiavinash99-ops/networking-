...
"""
Protocol handler for DNS communication (Variant 221)
...
class DNSPacketVariant221(BaseModel):
...
class DNSConnectionState221(BaseModel):
    state_id: int
    is_active: bool = True
    bytes_sent: int = 0
...
class DNSProtocolAnalyzer221:
    def __init__(self, interface: str):
        self.interface = interface
        self.active_connections: Dict[str, DNSConnectionState221] = {}
        self.packet_history: List[DNSPacketVariant221] = []
        
    def analyze_packet(self, raw_data: bytes) -> DNSPacketVariant221:
        # Simulate packet parsing
        parsed = DNSPacketVariant221(
    ...
    def _update_state(self, packet: DNSPacketVariant221):
        conn_key = f"{packet.source_ip}-{packet.destination_ip}"
        if conn_key not in self.active_connections:
            self.active_connections[conn_key] = DNSConnectionState221(state_id=len(self.active_connections))
...
def process_stream_221(stream_data: List[bytes]) -> Dict[str, Any]:
    analyzer = DNSProtocolAnalyzer221("eth0")
...
def apply_heuristics_221(stats: Dict[str, Any]) -> bool:
...
def format_report_221(stats: Dict[str, Any], malicious: bool) -> str:
    report = f"--- DNS Analysis Report (Var 221) ---\n"
