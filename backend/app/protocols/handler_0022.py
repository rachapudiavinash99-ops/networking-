...
"""
Protocol handler for SIP communication (Variant 22)
...
class SIPPacketVariant22(BaseModel):
...
class SIPConnectionState22(BaseModel):
    state_id: int
    is_active: bool = True
    bytes_sent: int = 0
...
class SIPProtocolAnalyzer22:
    def __init__(self, interface: str):
        self.interface = interface
        self.active_connections: Dict[str, SIPConnectionState22] = {}
        self.packet_history: List[SIPPacketVariant22] = []
        
    def analyze_packet(self, raw_data: bytes) -> SIPPacketVariant22:
        # Simulate packet parsing
        parsed = SIPPacketVariant22(
    ...
    def _update_state(self, packet: SIPPacketVariant22):
        conn_key = f"{packet.source_ip}-{packet.destination_ip}"
        if conn_key not in self.active_connections:
            self.active_connections[conn_key] = SIPConnectionState22(state_id=len(self.active_connections))
...
def process_stream_22(stream_data: List[bytes]) -> Dict[str, Any]:
    analyzer = SIPProtocolAnalyzer22("eth0")
...
def apply_heuristics_22(stats: Dict[str, Any]) -> bool:
...
def format_report_22(stats: Dict[str, Any], malicious: bool) -> str:
    report = f"--- SIP Analysis Report (Var 22) ---\n"
