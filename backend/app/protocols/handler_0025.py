...
"""
Protocol handler for SIP communication (Variant 25)
...
class SIPPacketVariant25(BaseModel):
...
class SIPConnectionState25(BaseModel):
    state_id: int
    is_active: bool = True
    bytes_sent: int = 0
...
class SIPProtocolAnalyzer25:
    def __init__(self, interface: str):
        self.interface = interface
        self.active_connections: Dict[str, SIPConnectionState25] = {}
        self.packet_history: List[SIPPacketVariant25] = []
        
    def analyze_packet(self, raw_data: bytes) -> SIPPacketVariant25:
        # Simulate packet parsing
        parsed = SIPPacketVariant25(
    ...
    def _update_state(self, packet: SIPPacketVariant25):
        conn_key = f"{packet.source_ip}-{packet.destination_ip}"
        if conn_key not in self.active_connections:
            self.active_connections[conn_key] = SIPConnectionState25(state_id=len(self.active_connections))
...
def process_stream_25(stream_data: List[bytes]) -> Dict[str, Any]:
    analyzer = SIPProtocolAnalyzer25("eth0")
...
def apply_heuristics_25(stats: Dict[str, Any]) -> bool:
...
def format_report_25(stats: Dict[str, Any], malicious: bool) -> str:
    report = f"--- SIP Analysis Report (Var 25) ---\n"
