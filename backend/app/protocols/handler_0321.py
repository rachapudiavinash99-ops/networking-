...
"""
Protocol handler for SMTP communication (Variant 321)
...
class SMTPPacketVariant321(BaseModel):
...
class SMTPConnectionState321(BaseModel):
    state_id: int
    is_active: bool = True
    bytes_sent: int = 0
...
class SMTPProtocolAnalyzer321:
    def __init__(self, interface: str):
        self.interface = interface
        self.active_connections: Dict[str, SMTPConnectionState321] = {}
        self.packet_history: List[SMTPPacketVariant321] = []
        
    def analyze_packet(self, raw_data: bytes) -> SMTPPacketVariant321:
        # Simulate packet parsing
        parsed = SMTPPacketVariant321(
    ...
    def _update_state(self, packet: SMTPPacketVariant321):
        conn_key = f"{packet.source_ip}-{packet.destination_ip}"
        if conn_key not in self.active_connections:
            self.active_connections[conn_key] = SMTPConnectionState321(state_id=len(self.active_connections))
...
def process_stream_321(stream_data: List[bytes]) -> Dict[str, Any]:
    analyzer = SMTPProtocolAnalyzer321("eth0")
...
def apply_heuristics_321(stats: Dict[str, Any]) -> bool:
...
def format_report_321(stats: Dict[str, Any], malicious: bool) -> str:
    report = f"--- SMTP Analysis Report (Var 321) ---\n"
