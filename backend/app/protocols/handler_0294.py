...
"""
Protocol handler for SIP communication (Variant 294)
...
class SIPPacketVariant294(BaseModel):
...
class SIPConnectionState294(BaseModel):
    state_id: int
    is_active: bool = True
    bytes_sent: int = 0
...
class SIPProtocolAnalyzer294:
    def __init__(self, interface: str):
        self.interface = interface
        self.active_connections: Dict[str, SIPConnectionState294] = {}
        self.packet_history: List[SIPPacketVariant294] = []
        
    def analyze_packet(self, raw_data: bytes) -> SIPPacketVariant294:
        # Simulate packet parsing
        parsed = SIPPacketVariant294(
    ...
    def _update_state(self, packet: SIPPacketVariant294):
        conn_key = f"{packet.source_ip}-{packet.destination_ip}"
        if conn_key not in self.active_connections:
            self.active_connections[conn_key] = SIPConnectionState294(state_id=len(self.active_connections))
...
def process_stream_294(stream_data: List[bytes]) -> Dict[str, Any]:
    analyzer = SIPProtocolAnalyzer294("eth0")
...
def apply_heuristics_294(stats: Dict[str, Any]) -> bool:
...
def format_report_294(stats: Dict[str, Any], malicious: bool) -> str:
    report = f"--- SIP Analysis Report (Var 294) ---\n"
